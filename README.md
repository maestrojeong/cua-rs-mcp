...
| **…and so does a terse button under a destructive question.** "OK" in a sheet asking *Delete 4 items?*, 확인 under *4개 항목을 삭제할까요?* — the verb is in the alert, not on the button. Only a sheet or dialog counts as the question; an ordinary window's content never does, so a mail thread about deleting nothing changes. **Cancel, No, 취소 are never refused** — backing out of a destructive dialog needs no confirmation. | on | same parameter |
