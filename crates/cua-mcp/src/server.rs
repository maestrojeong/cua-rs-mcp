...
fn render_popups(popups: &[cua_core::TransientWindow]) -> String {
    ...
    s.push_str(
        "  Accessibility cannot see inside these: they have no elements, no indices, and \
         `find` will not match their contents. Read them from the screenshot. To activate an \
         item, prefer press_key with the keyboard shortcut the item draws next to itself \
         (⌘I, ⌥⌘, and the like) — that goes through the app's own key handling and cannot \
         land one row off. Where there is no shortcut, click_in_window against the pop-up's \
         own window_id above, with x/y measured from ITS top-left corner, is the way in. \
         press_key `escape` dismisses one.\n",
    );
    s
}
